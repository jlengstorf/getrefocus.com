...
const __config = {
  ...
  classLink: 'floating-bar__nav-link',
  classLinkModifier: 'floating-bar__nav-item--current',
  triggerEl: 'hero',
};
const __sections = [];
...
class FloatingNav {
  ...
  constructor() {
    ...
    if (!!__floatingBar) {
      ...
      if (__triggerElement) {
        const triggerTop = __triggerElement.offsetTop;
        const triggerHeight = __triggerElement.offsetHeight;

        __triggerY = triggerTop + triggerHeight;
      } else {
        __triggerY = 0;
      }
  ...
  setBarVisibility() {
    const marginClass = '--hack_floating-bar__sibling--add-top-margin';

    let sibling = __floatingBar.nextElementSibling;
    while (sibling) {
      if (sibling.classList.contains('site-content__section')) {
        break;
      }
